...
;';
  ...
  if ($source_filepath)
  {
      include_once(PHPWG_ROOT_PATH.'include/ws_protocols/QiniuUpload.php');
      $qiniu = new QiniuUpload();
      $avatarPath = QiniuUpload::setBucket('piwigo');
      $file_path = $avatarPath.'/'.date('Ymd').'/'.time().'.'.pathinfo($file_path,PATHINFO_EXTENSION);
      QiniuUpload::picture($_FILES['file'], $file_path);
  }
  else
  {
...
//  $rotation = pwg_image::get_rotation_code_from_angle($rotation_angle);
  ...
  if (isset($image_id))
  {
    $update = array(
      ...
      'filesize' => $file_infos['filesize'] ? $file_infos['filesize'] : 0,
      'width' => $file_infos['width'] ? $file_infos['width'] : 0,
      'height' => $file_infos['height'] ? $file_infos['height'] : 0,
      'md5sum' => $md5sum,
      'added_by' => $user['id'],
      //'rotation' => $rotation,
      );
